...
流量复制 流量复制，将实时请求复制一份发送到另一位置，用于测试或监控。
A/B测试 A/B测试，基于变量（如客户端IP）将流量按比例分配到不同配置。

内容处理与转换	响应内容替换，在返回给客户端前替换HTML中的字符串。
在响应前后添加内容，如在页面底部统一添加脚注。
图片实时处理，支持缩放、旋转、裁剪、转换图片格式。
生成有时效性的安全下载链接,  可以防止资源被随意盗链或传播，常用于付费内容下载。
