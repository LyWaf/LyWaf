  ...
  <div class="space-y-4">
    ...
    <div class="card">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs text-gray-500">管理 IP 黑白名单和地理位置访问控制</span>
        </div>
    ...
    <div class="grid grid-cols-4 gap-3">
      <div class="card !py-3 text-center">
        <div class="text-2xl font-bold text-green-400">{{ whiteCount }}</div>
        <div class="text-xs text-gray-500">白名单</div>
      </div>
      <div class="card !py-3 text-center">
        <div class="text-2xl font-bold text-red-400">{{ blackCount }}</div>
        <div class="text-xs text-gray-500">黑名单</div>
      </div>
      <div class="card !py-3 text-center">
        <div class="text-2xl font-bold text-orange-400">{{ geoEntryCount }}</div>
        <div class="text-xs text-gray-500">地理位置规则</div>
      </div>
      <div class="card !py-3 text-center">
        <div class="text-2xl font-bold text-gray-100">{{ formatHit((data?.stats.blacklistBlockCount ?? 0) + (data?.stats.geoBlockCount ?? 0)) }}</div>
        <div class="text-xs text-gray-500">累计拦截</div>
      </div>
    </div>

    <!-- 开关 -->
    <div class="grid grid-cols-2 gap-3" v-if="data">
      ...
      <div class="card !py-3 flex items-center justify-between">
        <div>
          ...
          <div class="text-xs text-gray-500">启用地理位置访问限制（模式: {{ data.geoControl.mode === 'Deny' ? '禁止' : '允许' }}）</div>
        </div>
    ...
    <template v-else-if="data">
      <div v-if="entries.length === 0" class="card text-center py-12 text-gray-500">
        <p class="text-lg mb-2">暂无黑白名单规则</p>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-dark-border text-left text-xs text-gray-500">
                <th class="px-4 py-3 w-24">类型</th>
                <th class="px-4 py-3 w-24">分类</th>
            ...
            <tbody>
              <tr v-for="(entry, idx) in entries" :key="idx"
                ...
                <!-- 分类 -->
                <td class="px-4 py-3 text-gray-500 text-xs">{{ entry.category }}</td>
                <!-- 值 -->
                <td class="px-4 py-3 text-gray-200 font-mono text-sm">{{ entry.value }}</td>
                <!-- 操作 -->
                <td class="px-4 py-3">
